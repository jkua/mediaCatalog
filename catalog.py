    ...
    def catalog(self, path):
        for dirName, subdirList, fileList in os.walk(path):
            ...
            if len(filesToProcess) > 0:
                ...
                metadata = cataloger._getMetadata(filesToProcess)

                for file, md in zip(filesToProcess, metadata):
                    checksum = cataloger._md5sum(file)
...
extensions = ['cr2', 'cr3', 'jpg', 'jpeg', 'heif', 'mov', 'mp4', 'mp3', 'm4a']
...
if __name__=='__main__':
    ...
    cataloger = Cataloger(args.catalog)
